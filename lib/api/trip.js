...
exports.collection = function () {
	return db.query('SELECT * FROM trip').then(success,error);
};

exports.get = function (id) {
	return db.query('SELECT * FROM trip WHERE id = ' + id).then(success,error);
};
...
exports.create = function(location_id, month, year, duration, budget, image_url, description) {
	...
	db.query(insert,params).then(success,error);
};
